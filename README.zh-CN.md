...
> 本项目 Fork 自 [zarazhangrui/youtube-digest](https://github.com/zarazhangrui/youtube-digest)（MIT 许可），改名为 daweige digest。源码仓库仍是 [github.com/zarazhangrui/youtube-digest](https://github.com/zarazhangrui/youtube-digest)。

把 YouTube 和 Bilibili 视频变成一份可以深入学习的资料。daweige digest 把字幕、双语翻译、AI 概览、内容讲解和时间戳笔记放进同一个 Chrome 或 Edge 侧边栏，让你可以持续学习视频中的知识和语言，同时不丢失原视频上下文。

- 把零碎字幕变成清晰、可搜索的学习资料，YouTube 和 B 站同样好用。
...
- 支持 Bilibili：在 B 站视频页同样可以看字幕、AI 概览、选中文本讲解、时间戳笔记和翻译，这条路径不需要额外 API Key，只要浏览器里登录着 B 站。
- 界面完整本地化：整个侧边栏跟随设置页的语言选择（English / 简体中文），切换即时生效。
- 支持 Microsoft Edge：在 `edge://extensions` 加载同一份文件夹即可。
- 改名为 daweige digest；没有字幕的视频会显示清晰提示，不再报错。
...
不要把 API Key 发送到 AI 对话、源代码、截图或公开消息中。请你自己在 daweige digest 的设置页面直接填写。编程 Agent 可以告诉你填写位置，但不需要看到 Key。
...
4. 在 Chrome 地址栏打开 `chrome://extensions`。如果你使用 Microsoft Edge，请打开 `edge://extensions`。
5. 打开右上角的“开发者模式”。
6. 点击“加载已解压的扩展程序”。
7. 选择你刚才确定的那个准确项目文件夹，其中必须包含 `manifest.json`。
8. 如果需要，可以在浏览器扩展菜单中固定 daweige digest。

这是一个本地加载的扩展，不会自动更新。下载新版或让 Agent 修改代码后，请在 `chrome://extensions`（Edge 用户为 `edge://extensions`）中找到 daweige digest 并点击“重新加载”，然后刷新已经打开的 YouTube 页面。如果移动或删除源代码文件夹，Chrome 中加载的扩展会失效，需要从新的位置重新加载。

## 设置 API Key

daweige digest 最多需要你在自己的服务账号中准备两个 Key：

1. **DeepSeek API Key**，用于生成概览、讲解内容、翻译和自动润色笔记。
2. **Supadata API Key**，用于获取 YouTube 字幕。只有 YouTube 需要 Supadata；B 站字幕不需要这个 Key。
...
5. 复制 Key，并粘贴到 daweige digest 设置中的 **Supadata API key**。
...
3. 点击 **Create new API key**，填写容易识别的名称，例如 `daweige digest`，然后创建 Key。
4. 立即复制 Key。完整 Key 可能只会显示一次。
5. 把 Key 粘贴到 daweige digest 设置中的 **DeepSeek API key**。
6. 如果 DeepSeek 提示余额不足，请在 DeepSeek 开放平台账号中充值后再试。

当前账号和接口说明请查看 [DeepSeek 官方 API 文档](https://api-docs.deepseek.com/)。

在侧边栏中打开 **Settings**。你也可以在 `chrome://extensions` 的 daweige digest 卡片中打开扩展选项。Key 只能粘贴到这些设置输入框中。不要把 Key 发送到 AI 对话、项目文件、截图或公开消息中。
...
daweige digest 会让所有 DeepSeek 请求使用非思考模式，以获得更快、更稳定的交互。设置中的接口地址和模型固定，只需要填写 DeepSeek API Key。如果想使用其他服务或模型，请在设置中复制安全的自定义 prompt，让编程 Agent 修改你自己的本地副本。不要把任何 API Key 放进 prompt 或对话。

API Key 和设置保存在你设备上的 Chrome 扩展本地存储中。发布包不会包含或使用 `config.js`。

## 使用 daweige digest

1. 打开一个有字幕的普通 YouTube 视频页面，或一个 B 站视频页面。
2. 点击 daweige digest 扩展图标，打开侧边栏。
3. 阅读带时间戳的字幕，或选择 **Original**、**中文**、**双语**。
4. 打开 **Overview**，查看 AI 生成的章节和重点引用。
5. 选中字幕，获取 AI 内容讲解。
6. 从播放器或重点引用中保存笔记，之后可以在 **Notes** 中查看。

设置页提供 English 和中文两种界面语言。侧边栏、设置页和错误提示都跟随这一个选择。

## 当前支持范围

- Chrome 116 或更高版本。Microsoft Edge（Chromium 内核）可以通过 `edge://extensions` 加载本扩展。
- 标准的 `youtube.com/watch` 视频页面。
- B 站视频页面(`bilibili.com/video/...`)，包括多 P 视频。B 站字幕通过你浏览器里自己的 B 站登录状态获取，所以需要在浏览器中登录 B 站；这条路径不需要任何 API Key。
- Supadata 能够返回的原生字幕。daweige digest 会优先请求英文字幕，也可能显示其他可用的原生语言。
- 原文、简体中文和双语对照字幕。
- AI 概览、选中文本讲解、翻译和自动润色笔记。
- 本地笔记，以及最近字幕、概览和翻译的本地缓存。
- 发布版本的所有 AI 功能都使用 DeepSeek V4 Flash。其他服务需要修改本地代码，不属于发布版本的支持范围。

Shorts、直播、私密视频、受访问限制的视频，以及没有原生字幕的视频可能无法使用。B 站视频如果没有 CC 字幕或 AI 字幕，侧边栏会显示“无字幕”的中性提示，而不是字幕内容。目前没有测试 Firefox、Safari 或移动浏览器。

daweige digest 强制使用 Supadata 的 `mode=native`，不会在没有原生字幕时请求 AI 生成转录，也不会在本地转录音频。
...
- AI 生成字幕每分钟消耗 **2 credits**。daweige digest 不会使用这条路径，因为它强制使用 `mode=native`。
- 如果没有可用原生字幕并返回 HTTP `206`，仍会消耗 **1 credit**。

按照当前只获取原生字幕的方式，如果每次请求都成功，免费版每月大约可以查询 100 个视频。重试和没有字幕的查询也会消耗额度，所以实际成功数量可能更少。

DeepSeek 的额度与 Supadata 分开计算。daweige digest 不收款，也不转售 API 服务。建议为两个账号设置消费上限并定期查看用量。
...
daweige digest 使用原生 HTML、CSS 和 JavaScript，没有构建步骤，很适合用编程 Agent 做个人项目。你可以尝试：
...
如果想使用其他 AI 服务或模型，请先在编程 Agent 中打开 Chrome 通过“加载已解压的扩展程序”使用的那个准确的 daweige digest 项目文件夹。然后打开 daweige digest 设置并点击 **Copy customization prompt**。发送前替换 `[PROVIDER]` 和 `[MODEL]`，但不要加入任何 API Key。Agent 完成本地代码修改后，请你自己在它指出的设置位置填写 Key。

## 隐私和数据流向

daweige digest 会直接从扩展向服务商发送请求：
...
daweige digest 没有账号系统、广告、分析统计或行为追踪。Supadata 和 DeepSeek 仍会按照各自的条款和隐私政策处理数据。详情请查看 [PRIVACY.md](PRIVACY.md)。

## 常见问题

### YouTube 视频页面没有显示 Digest 按钮

- 在 `chrome://extensions` 中找到 daweige digest，点击“重新加载”，然后刷新 YouTube 页面。
...
- 在 `chrome://extensions` 中确认 daweige digest 已启用，并点击“重新加载”。
- 重新加载扩展后，刷新 YouTube 页面。
- 如果问题仍然存在，让你的编程 Agent 检查扩展。

### daweige digest 提示需要设置
